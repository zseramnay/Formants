...
def plot_results(df: pd.DataFrame, args):
    ...
    # ── Per-instrument literature reference table ─────────────────────────────
    INSTRUMENT_LIT = _build_instrument_lit()

    # Detect which instrument we're looking at from folder name + CSV codes
    folder_lower = Path(args.folder).name.lower()
    # Try to match from CSV instrument column first
    instr_codes = df_valid["instrument"].dropna().unique().tolist() if "instrument" in df_valid.columns else []

    lit_refs = {}
    matched_key = None
    for code in instr_codes:
        if code in INSTRUMENT_LIT:
            matched_key = code
            break
    # fallback: match by folder name substring
    if matched_key is None:
        for key in INSTRUMENT_LIT:
    ...
    if matched_key:
        ...
        # Build flat dict: label → hz (skip None values)
        for label, (f1, fp) in raw.items():
            if f1 is not None:
                lit_refs[label] = f1
        pipeline_fp_val = None
        for label, (f1, fp) in raw.items():
            if fp is not None:
                pipeline_fp_val = fp
                break
    else:
        # Generic fallback — no instrument-specific refs
        pipeline_fp_val = None

    # Assign colours and styles to however many lit_refs we have
    _lit_palette = ["#e74c3c", "#e67e22", "#27ae60", "#2980b9",
                    "#8e44ad", "#16a085", "#d35400", "#7f8c8d"]
    _lit_styles  = ["--", "-.", ":", (0,(3,1,1,1)), (0,(5,2)),
                    (0,(1,1)), (0,(3,5,1,5)), "--"]
    lit_colors = _lit_palette[:len(lit_refs)]
    lit_styles = _lit_styles[:len(lit_refs)]

    def add_lit_lines(ax, include_fp=False):
        for (label, val), col, ls in zip(lit_refs.items(), lit_colors, lit_styles):
            ax.axhline(val, linestyle=ls, color=col, linewidth=1.2,
                       alpha=0.9, label=label)
        if include_fp and pipeline_fp_val:
            ax.axhline(pipeline_fp_val, linestyle='-', color='#8e44ad',
                       linewidth=1.8, alpha=0.9, label=f"Pipeline Fp ({pipeline_fp_val} Hz)")
    ...
    ax.axhline(pipeline_fp_val, linestyle='-', color='#8e44ad', linewidth=1.5,
               label=f"Pipeline Fp ({pipeline_fp_val} Hz)") if pipeline_fp_val else None
    add_lit_lines(ax)
    ...
    if pipeline_fp_val:
        ax.axhline(pipeline_fp_val, linestyle='-', color='#8e44ad', linewidth=1.5,
                   label=f"Pipeline Fp ({pipeline_fp_val} Hz)", zorder=6)
    add_lit_lines(ax)
    ...
    if pipeline_fp_val:
        ax.axvline(pipeline_fp_val, linestyle='-', color='#8e44ad',
                   linewidth=1.8, label=f"Pipeline Fp ({pipeline_fp_val} Hz)")
...
# ─────────────────────────────────────────────────────────────────────────────
# Shared instrument literature table
# ─────────────────────────────────────────────────────────────────────────────

def _build_instrument_lit() -> dict:
    """
    Returns per-instrument literature reference values.
    Keys are SOL2020 instrument codes (from filename prefix).
    Values: { source_label: (F1_hz_or_None, Fp_hz_or_None) }
    Sources: Backus (1969), Meyer (2009), Giesler (1985),
             McCarty/CCRMA (2003), Référence Formantique pipeline.
    """
    return {
        # ── Brass ──────────────────────────────────────────────────────────────
        "TpC": {          # Trumpet in C
            ...
            "Meyer F1":     (800,  None),
            "Giesler F1":   (900,  None),
            "Pipeline F1":  (786,  None),
            "Pipeline Fp":  (None, 1046),
        },
        "Hn": {           # Horn in F
            "Backus F1":    (300,  None),
            "Meyer F1":     (340,  None),
            "Giesler F1":   (370,  None),
            "Pipeline F1":  (388,  None),
            "Pipeline Fp":  (None, 738),
        },
        "Tbn": {          # Tenor trombone
            "Backus F1":    (130,  None),
            "Meyer F1":     (200,  None),
            "Giesler F1":   (230,  None),
            "Pipeline F1":  (237,  None),
            "Pipeline Fp":  (None, None),
        },
        "BTb": {          # Bass tuba
            "Meyer F1":     (130,  None),
            "Giesler F1":   (180,  None),
            "Pipeline F1":  (226,  None),
            "Pipeline Fp":  (None, None),
        },
        ...
        "Fl": {           # Flute
            "Backus F1":    (1700, None),
            "Pipeline F1":  (743,  None),
            "Pipeline Fp":  (None, 1535),
        },
        "Ob": {           # Oboe
            "Backus F1":    (1300, None),
            "Giesler F1":   (1100, None),
            "Meyer F1":     (1100, None),
            "Pipeline F1":  (743,  None),
            "Pipeline Fp":  (None, 1485),
        },
        "Bn": {           # Bassoon
            "Backus F1":    (470,  None),
            "Giesler F1":   (500,  None),
            "Meyer F1":     (500,  None),
            "Pipeline F1":  (495,  None),
            "Pipeline Fp":  (None, 1079),
        },
        "ClBb": {         # Clarinet Bb
            "Backus F1":    (1500, None),
            "Giesler F1":   (1500, None),
            "Pipeline F1":  (463,  None),
            "Pipeline Fp":  (None, 1412),
        },
        "ClEb": {         # Clarinet Eb
            "Pipeline F1":  (678,  None),
            "Pipeline Fp":  (None, 1266),
        },
        "ClBs": {         # Bass clarinet
            "Pipeline F1":  (323,  None),
            "Pipeline Fp":  (None, None),
        },
        "Picc": {         # Piccolo
            "Backus F1":    (3400, None),
            "Pipeline F1":  (1109, None),
            "Pipeline Fp":  (None, 1320),
        },
        "FlBs": {         # Bass flute
            "Pipeline F1":  (301,  None),
            "Pipeline Fp":  (None, 1338),
        },
        "CA": {           # Cor anglais (English horn)
            "Backus F1":    (930,  None),
            "Meyer F1":     (600,  None),
            "Pipeline F1":  (452,  None),
            "Pipeline Fp":  (None, 1135),
        },
        "Cb": {           # Contrabassoon
            "Pipeline F1":  (226,  None),
            "Pipeline Fp":  (None, None),
        ...
        "Vn": {           # Violin
            "Meyer F1":     (300,  None),
            "Giesler F1":   (350,  None),
            "Pipeline F1":  (506,  None),
            "Pipeline Fp":  (None, 893),
        },
        "Va": {           # Viola
            "Meyer F1":     (280,  None),
            "Pipeline F1":  (377,  None),
            "Pipeline Fp":  (None, None),
        },
        "Vc": {           # Cello
            "Meyer F1":     (220,  None),
            "Giesler F1":   (250,  None),
            "Pipeline F1":  (205,  None),
            "Pipeline Fp":  (None, None),
        },
        "Cb": {           # Contrabass
            "Pipeline F1":  (172,  None),
            "Pipeline Fp":  (None, None),
        },
    }
...
def print_summary(df: pd.DataFrame, args):
    ...
    INSTRUMENT_LIT = _build_instrument_lit()
    folder_lower = Path(args.folder).name.lower()
    instr_codes = df["instrument"].dropna().unique().tolist() if "instrument" in df.columns else []
    matched_key = None
    for code in instr_codes:
        if code in INSTRUMENT_LIT:
            matched_key = code
