...
The Orchidea framework \citep{Cella2020,Cella2021}, a joint project between
IRCAM, HEM Geneva, and UC Berkeley continuing the Orchid* line of
computer-aided orchestration tools, addresses target-based orchestration through
a combination of stochastic matching pursuit, long short-term memory (LSTM)
neural networks, and monoobjective evolutionary optimization with a specifically
designed cost function. Target and database sounds are embedded in a
high-dimensional feature space; symbolic constraints (instruments, dynamics,
playing techniques) can be integrated into the cost function; and temporally
evolving sounds are handled by segmenting them into static targets optimized
jointly and then connected \citep{Cella2021}. Orchidea thus operates at the
level of global spectro-temporal similarity in a multi-feature space, seeking
the perceptually closest instrumental combination for a given target.

Our work complements this approach by operating at a different level of
description: rather than finding optimal combinations for a specific target
sound, we provide a systematic formant-level analysis that explains
\emph{why} certain instrument pairings produce perceptual fusion or
complementarity. The formant convergences identified here could potentially
inform Orchidea's cost function by providing acoustically grounded priors for
blend prediction.
